...
# ---------------------------------------------------------------------------
# Parse failure taxonomy for structured output extraction
# ---------------------------------------------------------------------------
...
def parse_phi_json(raw_output):
    ...
    # Strip markdown code fences from entire output before any parsing.
    # Models (especially Gemma) wrap JSON in ```json ... ``` blocks.
    raw = re.sub(r'```(?:json)?\s*', '', raw).strip()

    # Re-check for empty after fence stripping
    if raw in ("[]", "", "None", "No PHI found", "No PHI found.",
                "No PHI", "none", "NONE"):
        return []
    ...
    # Try comma-joining adjacent JSON objects and wrapping in array.
    # After fence stripping, output may be: {"text":"a","type":"NAME"}\n{"text":"b","type":"DATE"}
    ...
    # Extract all JSON objects via regex as final fallback
    results = []
...
def extract_cot_output(raw_output, original_query=""):
    """Extract de-identified text from structured multi-section output.

    Implements cascading fallback for malformed or truncated responses.
