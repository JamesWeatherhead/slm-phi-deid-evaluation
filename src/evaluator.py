...
def evaluate_query(query, raw_output, parse_status=None):
    ...
    Args:
        ...
        parse_status: ParseResult value for structured outputs
...
def evaluate_responses_file(responses_path, dataset, output_dir=None):
    ...
    for resp in responses:
        ...
        # Use extracted_output if available (structured parsers),
        # otherwise use raw_output (which is pass2 output for P4, direct for others)
...
def aggregate_evaluation(eval_results, model_slug, prompt_slug):
    ...
    # Tier 2 precision per run (of all tags placed, how many matched actual PHI?)
    t2_precision_per_run = []
    for run in runs:
        run_pos = [r for r in positive if r.get("run_number") == run]
        if run_pos:
            total_tags_placed = sum(len(r.get("all_detected_tags", [])) for r in run_pos)
            total_matched = sum(r["tier2"]["phi_detected"] for r in run_pos)
            if total_tags_placed > 0:
                t2_precision_per_run.append(total_matched / total_tags_placed)

    # Tier 3 fidelity pass rate per run
    t3_pass_per_run = []
    t3_similarity_per_run = []
    for run in runs:
        ...
        if run_all:
            ...
            sims = [r["tier3"]["levenshtein_similarity"] for r in run_all
                    if r["tier3"].get("levenshtein_similarity") is not None]
            if sims:
                t3_similarity_per_run.append(float(np.mean(sims)))
    ...
    return {
        ...
        "positive_queries": {
            ...
            "tier2": {
                ...
                "precision_mean": _safe_mean(t2_precision_per_run),
                "precision_std": _safe_std(t2_precision_per_run),
                "precision_per_run": [round(r, 4) for r in t2_precision_per_run],
            },
            "tier3": {
                "fidelity_pass_rate_mean": _safe_mean(t3_pass_per_run),
                "fidelity_pass_rate_per_run": [round(r, 4) for r in t3_pass_per_run],
                "similarity_mean": _safe_mean(t3_similarity_per_run),
                "similarity_std": _safe_std(t3_similarity_per_run),
                "similarity_per_run": [round(r, 4) for r in t3_similarity_per_run],
            },
            "per_category": {
                cat: {
