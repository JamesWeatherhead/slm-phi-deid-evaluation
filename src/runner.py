...
from src.utils import (
    ...
    parse_phi_json, apply_redactions,
...
def query_ollama(host, model_tag, system_prompt, user_prompt, params,
    ...
    # Apply presence_penalty if specified in model overrides
...
def run_single_query(host, model_tag, model_slug, prompt_config, prompt_slug,
    ...
    # Model overrides applied first, then prompt overrides applied on top.
    ...
    if parser == "json_then_replace":
        ...
        if result["error"]:
            ...
            record["raw_output"] = parse_direct_output(result["raw_output"])
            record["error"] = None
...
def get_primary_prompt_slugs(templates):
    """Get the primary prompt slugs (no ablations)."""
    primary = [
        "zero-shot-minimal",
...
def main():
    ...
    parser.add_argument(
        ...
        help="Model slug (phi4-mini, llama32-3b, gemma3-4b). Required unless --all is set."
    )
    parser.add_argument(
